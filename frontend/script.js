...
const API_BASE_URL =
  window.location.hostname === "localhost"
    ...
    : "https://document-intelligence-backend-1y0j.onrender.com"; // Replace with actual backend URL
...
document.getElementById("upload-form").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (response.ok) {
      showResult(
        "success",
        `Document processed successfully: ${data.document_name}`,
      );
      fileInput.value = "";
    } else {
...
async function loadDocuments() {
  ...
  try {
    ...
    if (documents.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="5" class="loading">No documents processed yet</td></tr>';
      return;
    }
...
async function viewDocument(documentName) {
  ...
  try {
    ...
    console.log(
      "Fetching document details from:",
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to load document details: ${response.status} ${response.statusText}`,
      );
    }
...
function renderDocumentDetails(data) {
  ...
  if (extracted.invoice_number)
    fields.push(renderField("Invoice Number", extracted.invoice_number));
  if (extracted.invoice_date)
    fields.push(renderField("Invoice Date", extracted.invoice_date));
  if (extracted.vendor_name)
    fields.push(renderField("Vendor Name", extracted.vendor_name));
  if (extracted.currency)
    fields.push(renderField("Currency", extracted.currency));
  if (extracted.subtotal)
    fields.push(renderField("Subtotal", extracted.subtotal));
  if (extracted.tax_amount)
    fields.push(renderField("Tax Amount", extracted.tax_amount));
  if (extracted.discount)
    fields.push(renderField("Discount", extracted.discount));
  if (extracted.total_amount)
    fields.push(renderField("Total Amount", extracted.total_amount));

  // Balance sheet fields
  if (extracted.total_assets)
    fields.push(renderField("Total Assets", extracted.total_assets));
  if (extracted.total_liabilities)
    fields.push(renderField("Total Liabilities", extracted.total_liabilities));
  if (extracted.total_equity)
    fields.push(renderField("Total Equity", extracted.total_equity));

  // Profit & Loss fields
  if (extracted.revenue) fields.push(renderField("Revenue", extracted.revenue));
  if (extracted.expenses)
    fields.push(renderField("Expenses", extracted.expenses));
  if (extracted.net_profit)
    fields.push(renderField("Net Profit", extracted.net_profit));

  // Cash flow fields
  if (extracted.operating_cash_flow)
    fields.push(
      renderField("Operating Cash Flow", extracted.operating_cash_flow),
    );
  if (extracted.investing_cash_flow)
    fields.push(
      renderField("Investing Cash Flow", extracted.investing_cash_flow),
    );
  if (extracted.financing_cash_flow)
    fields.push(
      renderField("Financing Cash Flow", extracted.financing_cash_flow),
    );
  if (extracted.net_cash_flow)
    fields.push(renderField("Net Cash Flow", extracted.net_cash_flow));
  ...
  if (
    ...
    data.validation.checks.length > 0
  ) {
    const validationsHtml = data.validation.checks
      .map(
        (v) => `
            <div class="validation-item ${v.status}">
                ...
                <div class="validation-details">
                    ${Object.entries(v.operands)
                      .map(
                        ([k, val]) =>
                          `<div>${escapeHtml(k)}: ${escapeHtml(String(val))}</div>`,
                      )
                      .join("")}
    ...
    document.getElementById("financial-validations").innerHTML =
      "<p>No financial validations performed</p>";
  }
